...
def model():
    ...
    W2 = tf.Variable(tf.truncated_normal([hidden_layer_nodes_1, categories], stddev=0.1))
    ...
    data_x = dataX(features, r'dataset\train\[0-3]')
    print("datax: ", data_x)
    data_y = dataY(categories, r'dataset\train\[0-3]')
    print("datay: ", data_y)
    data_x_test = dataX(features, r'dataset\test\[0-3]')
    data_y_test = dataY(categories, r'dataset\test\[0-3]')
    data_x_validation = dataX(features, r'dataset\validation\[0-3]')
    data_y_validation = dataY(categories, r'dataset\validation\[0-3]')
    ...
    for i in range(len(data_x_test)):
        print('Prediction for: "', data_x_test[i], '": ', sess.run(y, feed_dict={x: [data_x_test[i]]}),
              ',  Max value: ', max(sess.run(y, feed_dict={x: [data_x_test[i]]})[0]), ',  Sum: ',
              sum(sess.run(y, feed_dict={x: [data_x_test[i]]})[0]), ', real class: ', data_y_test[i])
