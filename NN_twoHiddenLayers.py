...
def model():
    ...
    data_x = np.load(r't_x.npy')  # dataX(features, r'dataset\train\[0-42]')
    data_x = data_x.astype(int)
    print("datax: ", data_x)
    data_y = np.load(r't_y.npy')  # dataY(categories, r'dataset\train\[0-42]')

    print("datay: ", data_y)
    data_x_test = np.load(r'test_x.npy')  # dataX(features, r'dataset\test\[0-42]')
    data_x_test = data_x_test.astype(int)
    data_y_test = np.load(r'test_y.npy')  # dataY(categories, r'dataset\test\[0-42]')
    data_y_test = data_y_test.astype(int)
    data_x_validation = np.load(r'v_x.npy')  # dataX(features, r'dataset\validation\[0-42]')
    data_x_validation = data_x_validation.astype(int)
    data_y_validation = np.load(r'v_y.npy')  # dataY(categories, r'dataset\validation\[0-42]')
